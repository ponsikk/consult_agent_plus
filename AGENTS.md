...
## ⛔ АБСОЛЮТНЫЙ ЗАПРЕТ — GIT

**НЕ ИСПОЛЬЗУЙ GIT ВООБЩЕ.** Никогда. Совсем.

Запрещены любые git-команды: `git add`, `git commit`, `git push`, `git status`, `git log`, `git diff`, `git checkout`, `git stash`, `git config` — и любые другие.

Твоя работа — редактировать файлы напрямую через инструменты (Edit, Write, Read).
Git — исключительная зона тимлида. Если ты используешь git — ты нарушаешь главное правило.
...
- Менять `git config` (user.name, user.email и любые другие параметры) — не трогать никогда
- Делать `git commit`, `git add` и любые другие git-команды — не делать никогда. Только редактируй файлы. Git — зона тимлида.
