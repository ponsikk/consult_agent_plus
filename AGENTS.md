...
## Правило 1: Source Control & Release
- **ЗАПРЕЩЕНО** делать `git push` агентам.
- **Процесс:** 
  1. Исполнитель (Backend/Frontend/DevOps) делает `git commit` локально.
  2. **AGENT_QA** проверяет изменения (тесты, линт, логи).
  3. **AGENT_QA** выносит вердикт в `qa_report.md`: "READY FOR PUSH".
  4. **Тимлид** выполняет `git push` из своего терминала.
